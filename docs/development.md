...
Documentation builds require Doxygen 1.18 or newer, Graphviz (`dot`), and npm.
The pinned HTML template targets the Doxygen 1.18 JavaScript runtime; older
generators are rejected during configuration to avoid broken navigation.
These tools are not required when configuring with `-DBUILD_DOCS=OFF`.
